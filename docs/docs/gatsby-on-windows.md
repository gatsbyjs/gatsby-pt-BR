...
Se a instação das dependências ou o desenvolvimento no Windows lhe der qualquer problema, Windows 10 fornece uma ótima alternativa: [Windows Subsystem for Linux(WSL)](https://docs.microsoft.com/en-us/windows/wsl/about). Permite rodar a maioria dos ferramentas, utilitarios e aplicações de linha de comando em um ambiente GNU/Linux diretamente no Windows, sem modificações e sem a sobrecarga de uma maquina virtual. Para mais instruções de como usar WSL, veja [Gatsby no Linux](/docs/gatsby-on-linux/#windows-subsystem-linux-wsl).