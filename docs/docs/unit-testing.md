...
---
Testes unitários são uma ótima forma de se proteger contra erros no seu código antes de você implantá-lo. Embora o Gatsby nao tenha suporte nativo aos testes unitários, basta apenas alguns passos
...
A mais popular framework de testes para React é a [Jest](https://jestjs.io/),
a qual foi criada pelo Facebook. Mesmo Jest sendo uma framework de testes unitários
de uso geral no JavaScript, ela tem muitos recursos que funcionam particularmente bem
com React.
...
Primeiro você precisa instalar a Jest e alguns pacotes necessários. Nós instalamos
...
- A próxima opção é `moduleNameMapper`. Esta seção funciona um pouco com as
 regras do Webpack, e diz ao Jest como tratar as importações. Você deve se
 atentar principalmente à simulação de importação de arquivos estáticos,
 a qual o Jest não consegue fazer. A simulação  é um módulo fictício que
 ...
 coisa, e aqui você está simulando assets em vez do código. Para stylesheets
 você precisa utilizar o pacote `identity-obj-proxy`. Para todos os outros assets
 você precisa usar manualmente uma simulação chamada `file-mock.js`. Você precisa
...
- A próxima configuração é `testPathIgnorePatterns`. Você está falando ao Jest para
 ignorar quaisquer testes nos diretórios `node_modules` or `.cache`. 
...
- Há mais uma configuração global que você precisa definir, mas é uma função
 você não pode definir aqui no JSON. O array `setupFiles` permite você listar
...
### 3. Simulações úteis para completar seu ambiente de testes
...
testar compononentes que usam `Link` or GraphQL.
...
pode começar com um simples teste de snapshot para checar se está tudo funcionando.
...
Este é um teste de snapshot muito simples, que usa `react-test-renderer` para renderizar
o componente e, em seguida, gera um snapshot dele na primeira execução. Isso então
faz a comparação com futuros snapshots, o que significa que você pode checar rapidamente
as regressões. Visite [A documentacao do Jest](https://jestjs.io/docs/en/getting-started)
...
um script para `testes`, que apenas gera uma mensagem de erro. Mude para utilizar
...
Atualiza a configuração no `jest.config.js`, da seguinte forma:
...
A única coisa coisa que está sendo feita é dizer ao Jest quais extensões
...
embora lembre-se de que você pode instalar as versões do Babel 7. Acesse [the Gatsby
Babel config guide](/docs/babel) para alguns exemplos.
 
Para mais informações sobre teste no Jest, visite [the Jest site](https://jestjs.io/docs/en/getting-started).
